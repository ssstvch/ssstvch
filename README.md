### Используемые языки и технологии

<div align=center>
<img src="https://raw.githubusercontent.com/devicons/devicon/1119b9f84c0290e0f0b38982099a2bd027a48bf1/icons/javascript/javascript-plain.svg" width="40">
    <img src="https://raw.githubusercontent.com/devicons/devicon/1119b9f84c0290e0f0b38982099a2bd027a48bf1/icons/sass/sass-original.svg" width="40">
    <img src="https://mobx.js.org/img/mobx.png" width="40">
    <img src="https://raw.githubusercontent.com/devicons/devicon/1119b9f84c0290e0f0b38982099a2bd027a48bf1/icons/react/react-original.svg" width="40">
    <img src="data:image/svg+xml;base64," width="40"/>
    <img src="https://react-bootstrap-table.github.io/react-bootstrap-table2/img/logo/pure-color-square.svg" width="40" />
     <img src="https://avatars.githubusercontent.com/u/18673496?s=280&v=4" width="40">
    <img src="https://date-fns.org/static/7ceafe855b131d457fe7de810ed31e0f.svg" width="40" />

</div>

<br />

- Javascript
- SCSS
- MobX (https://mobx.js.org/README.html)
- React (https://ru.reactjs.org/)
  - react-bootstrap (https://react-bootstrap.github.io/)
  - react-bootstrap-table-next, react-bootstrap-table2-filter/paginator/toolkit (https://react-bootstrap-table.github.io/react-bootstrap-table2/)
  - localForage (https://localforage.github.io/localForage/)
  - date-fns (https://date-fns.org/)
  - mobx-react

---

### Запуск проекта

<p>Для запуская проекта потребуется установленный Node.js и npm.</p> 
<p>Находясь в папке проекта "source" в IDE, в терминале ввести команду</p>
<code>npm install</code>
<br />
<br />
<p>чтобы загрузить все зависимости. После успешной загрузки, не выходя из папки в терминале ввести команду </p>
<code>npm start</code>
<br />
<br />
<p>При необходимости, согласиться сменить порт.</p>

---

### Данные стажера

<p>Ситович Александра Анатольевна</p>
<div id="badges">
    <a href="https://t.me/ssstvch">
        <img src="https://img.shields.io/badge/Telegram-blue?style=social&logo=telegram&logoColor=gray" alt="Telegram Badge"/> 
    </a>
    <a href="https://t.me/ssstvch">
        <img src="https://img.shields.io/badge/ssstvch-blue?style=social&logo=git&logoColor=gray" alt="Github Badge"/> 
    </a>
</div>
